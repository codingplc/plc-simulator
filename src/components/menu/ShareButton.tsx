...
const ShareButton: React.FC<Props> = (props: Props) => {
  ...
  const share = () => {
    ...
    batch
      .commit()
      .then(() => {
        const projectsCreated = parseInt(localStorage.getItem("ph_projects_created") || "0") + 1;
        localStorage.setItem("ph_projects_created", projectsCreated.toString());
        const projectsShared = parseInt(localStorage.getItem("ph_projects_shared") || "0") + 1;
        localStorage.setItem("ph_projects_shared", projectsShared.toString());
        posthog.capture("project_created", {
          $set: { projects_created: projectsCreated, projects_shared: projectsShared, email: user.email },
        });
